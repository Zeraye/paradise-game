...
from obstacles import ObstacleSquare, ObstacleCross, ObstacleBigCross, ObstacleLines
...
def main(win):
    ...
    reset_board()
    p.p_update_pos(5, 5)
    last_obstacle_time = last_obstacle_time2 = variables.start_game_time = time.time()
    max_width = 2
    time_between_obstacles = 1.2
    time_between_obstacles2 = 0.8
    run = True
    while run:
        curr_time = time.time()
        ...
        if time.time() - variables.start_game_time > 120:
            ...
            win_menu_func(win)
        if curr_time - last_obstacle_time2 > time_between_obstacles2 and not variables.death:
            last_obstacle_time2 = time.time()
            new_obstacle = ObstacleLines(curr_time, variables.delay, random.randint(1, max_width))
